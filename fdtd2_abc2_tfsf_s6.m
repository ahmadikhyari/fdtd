...
[X, Y] = meshgrid(x,y);
trig = (Y+X-201*dx+dx < 0) .* (X-101*dx+dx < 0);
for I = 1:i
    for J = 1:j
        if trig(J,I) == 1
            mu2(J,I) = 1*mu0; ep2(J,I) = 9*ep0;
            sigm2(J,I) = 0; sige2(J,I) = 1e-2;
        end
    end
end
...
is = 1; js = 1;
ir = 101; jr = 101;
mu2(js:jr,is:ir) = 1*mu0; ep2(js:jr,is:ir) = 4*ep0;
sigm2(js:jr,is:ir) = 0; sige2(js:jr,is:ir) = 0;
...
for n=0:1000
    Ezincnow(ys,xs) = amp*cos(2*pi*f*n*dt-2*pi/lambda*z);
    ...
    Eznow(ys,xs) = Ezincnow(ys,xs);
    Eznow2(ys,xs) = Ezincnow(ys,xs);
    ...
    set(gcf,'Color','black');
    set(gcf,'InnerPosition',[0 0 720 720]);
    
    colormap(gray);
    
    subplot(2,2,1);
    imagesc(x,y,((Eznew)),[-amp amp]);
    axis('square');
    set(gca,'XColor','white');
    set(gca,'YColor','white');
    
    subplot(2,2,2);
    imagesc(x,y,((Ezincnew)),[-amp amp]);
    axis('square');
    set(gca,'XColor','white');
    set(gca,'YColor','white');
    
    subplot(2,2,3);
    imagesc(x,y,((Eznew2)),[-amp amp]);
    axis('square');
    set(gca,'XColor','white');
    set(gca,'YColor','white');
    
    subplot(2,2,4);
    imagesc(x,y,((Eznew2-Eznew)),[-amp amp]);
    axis('square');
    set(gca,'XColor','white');
    set(gca,'YColor','white');
